...
context '新規登録できない場合' do
    ...
    it 'product_nameは40字以上では登録できない'do
        @item.product_name =  Faker::Internet.text( max_length: 41)
        @item.valid?
        expect(@item.errors.full_messages).to include('product_name is too long (minimum is 41 characters)')
  end
    ...
    it "product_explanationが1000字以上では登録できない" do
      @item.product_explanation = Faker::Internet.text(minimum_length: 1001)
      @item.valid?
